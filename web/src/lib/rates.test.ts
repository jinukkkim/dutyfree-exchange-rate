...
test("고시가 멈춘 채 맞은 주말은 확정이 아니다", () => {
  ...
  // 첫 주말이 10-03(토) 이다 — 그 전 토요일 09-26 은 4일(추석 휴장 제외)이라 아직 신선하고,
  // 평일에 잡으면 "오늘 고시가 아직 없다"는 기존 분기로도 false 가 나와
  // 이 가드가 근거인지 구별되지 않는다.
  expect(isTomorrowConfirmed(parseRates(CSV), "2026-10-03")).toBe(false)
})

test("평일 휴장일에는 직전 고시가 내일로 확정된다", () => {
  // 2026 추석 09-24(목)·25(금). 09-23 고시가 24~28일에 이어서 적용된다.
  const rates = parseRates(`${CSV}2026-09-23,1360,MAR,smbs,2026-09-23T09:00:00+09:00\n`)
  for (const holiday of ["2026-09-24", "2026-09-25"]) {
    expect(isTomorrowConfirmed(rates, holiday)).toBe(true)
    expect(appliedOn(rates, nextDay(holiday))?.rate).toBe(1360)
  }
})
